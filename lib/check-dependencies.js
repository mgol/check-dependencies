...
function checkDependenciesHelper(syncOrAsync, config, callback) {
    ...
    function checkPackage(name, versionString, isOptional) {
        ...
        if (!semver.satisfies(depVersion, versionString)) {
            success = false;
            logs.fail.push(name + ': installed: ' + chalk.red(depVersion) +
                ', expected: ' + chalk.green(versionString));
            // error(name + ': installed: ' + chalk.red(depVersion) +
            //     ', expected: ' + chalk.green(versionString));
        } else {
            logs.success.push(name + ': installed: ' + chalk.green(depVersion) +
                ', expected: ' + chalk.green(versionString));
    ...
    log('');
    log('=== success ==='.green);
    _.forEach(logs.success, function (msg) {
      log( msg );
    });
    log('');
    log('=== fail ==='.red);
    // display fail
    _.forEach(logs.fail, function (msg) {
      error( msg );
    });
    log('');
