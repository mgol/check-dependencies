...
function checkDependenciesHelper(config, syncOrAsync, callback) {
    ...
    function installOrPrune(mode) {
        ...
        var msg,
            method = syncOrAsync === 'sync' ? spawnSync : spawn,
            spawnReturn = method(win32 ? (pkgManagerPath ? 'node' : 'cmd') : options.packageManager,
                (win32 ?
...
module.exports = function checkDependencies(config, callback) {
    return checkDependenciesHelper(config, 'async', callback);
};
...
module.exports.sync = function checkDependenciesSync(config) {
    ...
    return checkDependenciesHelper(config, 'sync');
};
