...
module.exports = function checkDependencies(config, callback) {
    ...
    if (options.type === 'bower') {
        // Allow a local bower.
        pkgManagerPath = findup('node_modules/.bin/bower');
    }
    ...
    spawn(win32 ? (pkgManagerPath ? 'node' : 'cmd') : options.type,
        win32 ? (pkgManagerPath ? [] : ['/c']).concat([options.type, 'install']) : ['install'],
        {
